...
class WebTaskItemsEditingTest < ActionDispatch::IntegrationTest
  test "guest tries to access new task form" do
    get(web_adapter.edit_task__item_url(task_lists(:one_inbox), task_items(:one)))
  ...
  test "guest tries to create a task" do
    put(
      web_adapter.task__item_url(task_lists(:one_inbox), task_items(:one)),
  ...
  test "user tries to update a task from another user" do
    ...
    put(web_adapter.task__item_url(task.task_list, task), params: { task_item: { name: "Foo" } })
  ...
  test "user updates a task with valid data" do
    ...
    get(web_adapter.edit_task__item_url(task.task_list, task))
  ...
  test "user updates a task with invalid data" do
    ...
    put(web_adapter.task__item_url(task.task_list, task), params: { task_item: { name: "" } })
