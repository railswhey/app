...
class APIV1UserPasswordsTest < ActionDispatch::IntegrationTest
  test "#create responds with 400 when params are missing" do
    params = [ {}, { user: {} }, { user: nil } ].sample

    post(api_v1_adapter.user__passwords_url, params:)
  ...
  test "#create responds with 200 when the email is not found" do
    ...
    post(api_v1_adapter.user__passwords_url, params: { user: { email: } })
  ...
  test "#update responds with 422 when token is invalid" do
    ...
    put(api_v1_adapter.user__password_url(token, format: :json), params:)

    api_v1_adapter.assert_response_with_failure(:unprocessable_entity) => { message: }
