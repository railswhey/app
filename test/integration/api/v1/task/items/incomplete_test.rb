...
class APIV1TaskItemsIncompleteTest < ActionDispatch::IntegrationTest
  test "#update responds with 401 when API token is invalid" do
    ...
    put(api_v1_adapter.incomplete_task__item_url(member!(user).inbox, task, format: :json), headers:)
  ...
  test "#update responds with 404 when task list is not found" do
    ...
    url = api_v1_adapter.incomplete_task__item_url(Workspace::List.maximum(:id) + 1, task.id, format: :json)
  ...
  test "#update responds with 404 when task is not found" do
    ...
    url = api_v1_adapter.incomplete_task__item_url(member!(user).inbox, Workspace::Task.maximum(:id) + 1, format: :json)
  ...
  test "#update responds with 404 when task list belongs to another user" do
    ...
    put(
      api_v1_adapter.incomplete_task__item_url(task.list, task, format: :json),
  ...
  test "#update responds with 200 when task is marked as completed" do
    ...
    assert_changes -> { task.reload.completed_at } do
      put(
        api_v1_adapter.incomplete_task__item_url(member!(user).inbox, task, format: :json),
        headers: api_v1_adapter.authorization_header(user)
      )
    end
