...
class APIV1TaskItemsIncompleteTest < ActionDispatch::IntegrationTest
  ...
  test "#update responds with 404 when task list is not found" do
    ...
    url = api_v1_adapter.incomplete_task__item_url(Task::List.maximum(:id) + 1, task.id, format: :json)
  ...
  test "#update responds with 404 when task is not found" do
    ...
    url = api_v1_adapter.incomplete_task__item_url(member!(user).inbox, Task::Item.maximum(:id) + 1, format: :json)
  ...
  test "#update responds with 404 when task list belongs to another user" do
    ...
    put(
      api_v1_adapter.incomplete_task__item_url(task.task_list, task, format: :json),
