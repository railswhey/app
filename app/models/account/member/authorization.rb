...
class Account::Member::Authorization
  ...
  def find_user
    account_id = member.task_list_id ? "task_lists.account_id" : "account_memberships.account_id"
  ...
  def users_left_joins
    ...
    memberships_condition = "AND account_memberships.#{membership_assignment}" if membership_assignment

    User
      .joins("LEFT JOIN account_memberships ON users.id = account_memberships.user_id #{memberships_condition}")
      .joins("LEFT JOIN task_lists ON task_lists.account_id = account_memberships.account_id AND #{task_lists_condition}")
  end
